    ...
    <div className="auth-premium-layout" style={{ display: 'flex', minHeight: '100vh', background: 'var(--color-ink)', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="auth-premium-left" style={{ flex: 1, position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', padding: '48px', color: 'white', background: 'linear-gradient(135deg, var(--color-ink) 0%, #1a1400 60%, #2a1f00 100%)' }}>
        ...
        <div style={{ marginTop: 'auto', marginBottom: 'auto', zIndex: 10, maxWidth: '480px' }}>
          <h1 style={{ fontSize: '56px', fontWeight: 900, lineHeight: 1.1, marginBottom: '24px', letterSpacing: '-1px' }}>Find your way back.</h1>
          <p style={{ fontSize: '20px', color: 'rgba(255,255,255,0.7)', fontWeight: 500, lineHeight: 1.5 }}>It happens to the best of explorers. Let's get you back on track.</p>
        </div>
      </div>
      ...
      <div className="auth-premium-right" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--color-surface)', padding: '48px' }}>
        <div className="auth-premium-glass" style={{ width: '100%', maxWidth: '440px', background: 'var(--color-surface)', padding: '48px', borderRadius: '24px', boxShadow: '0 24px 48px rgba(0,0,0,0.05)', border: '1px solid var(--color-border-light)' }}>
          <h2 style={{ fontSize: '32px', fontWeight: 900, marginBottom: '8px', color: (typeof loginStatus !== 'undefined' && loginStatus === 'success') || (typeof registerStatus !== 'undefined' && registerStatus === 'success') || (typeof status !== 'undefined' && status === 'loading') ? 'var(--color-gold)' : 'var(--color-text-primary)', letterSpacing: '-0.5px' }}>Reset password</h2>
          ...
          {status === 'success' ? (
            <div style={{ background: 'var(--color-surface-2)', padding: '24px', borderRadius: '16px', border: '1px solid var(--color-border)', textAlign: 'center' }}>
              ...
              <h3 style={{ margin: '0 0 8px 0', fontSize: '18px', fontWeight: 800, color: (typeof loginStatus !== 'undefined' && loginStatus === 'success') || (typeof registerStatus !== 'undefined' && registerStatus === 'success') || (typeof status !== 'undefined' && status === 'loading') ? 'var(--color-gold)' : 'var(--color-text-primary)' }}>Check your inbox</h3>
              <p style={{ margin: 0, fontSize: '14px', color: 'var(--color-text-secondary)' }}>We've sent a recovery link to <strong>{email}</strong>.</p>
              <Link to="/login" style={{ display: 'inline-block', marginTop: '24px', color: 'var(--color-gold-dark)', fontWeight: 700, textDecoration: 'none' }}>
                Return to log in
              </Link>
            </div>
          ) : (
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              <div>
                ...
                <input 
                  ...
                  required 
                  style={{ width: '100%', padding: '14px 16px', borderRadius: '12px', border: '1px solid var(--color-border)', fontSize: '15px', outline: 'none', transition: 'border-color 0.2s' }} 
          ...
          <div style={{ marginTop: '32px', textAlign: 'center', fontSize: '14px', color: 'var(--color-text-secondary)', fontWeight: 500 }}>
            Remembered your password? <Link to="/login" style={{ color: (typeof loginStatus !== 'undefined' && loginStatus === 'success') || (typeof registerStatus !== 'undefined' && registerStatus === 'success') || (typeof status !== 'undefined' && status === 'loading') ? 'var(--color-gold)' : 'var(--color-text-primary)', fontWeight: 800, textDecoration: 'none' }}>Log in</Link>
          </div>
        </div>
      </div>
    </div>
